...
class MXMLCHelperTest <  Test::Unit::TestCase
  ...
  def test_test_helper
    ...
    assert_equal('-default-background-color=#FFFFFF -default-frame-rate=24 -default-size 1000 550 -library-path+=bar -output=bin/SomeProjectRunner.swf -source-path+=src -source-path+=foo -source-path+=assets -source-path+=test -source-path+=lib/asunit3 -verbose-stacktraces=true -warnings=true src/SomeProjectRunner.as', t.to_shell)
  end
